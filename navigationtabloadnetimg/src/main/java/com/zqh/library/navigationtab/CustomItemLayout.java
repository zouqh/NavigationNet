...
public class CustomItemLayout extends ViewGroup implements ItemController {
    ...
    public void initialize(List<BaseItem> items, boolean animateLayoutChanges) {
        mItems.clear();
        mItems.addAll(items);
        ...
        int n = mItems.size();
        for (int i = 0; i < n; i++) {
            final BaseItem tabItem = mItems.get(i);
...
//    }
    ...
    @Override
    public void addCustomItem(int index, final BaseItem item) {
    ...
    public void setDefaulUrl(int index, String url) {
        mItems.get(index).setDefaultIconUrl(url);
    }

    @Override
    public void setSelectUrl(int index, String url) {
        mItems.get(index).setSelectedIconUrl(url);
    }
